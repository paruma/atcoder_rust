...
4. **Clippy による静的解析**:
    - 以下のコマンドで編集中のファイルに関する指摘がないか確認してください。
      ```bash
      cargo clippy -p mylib --all-targets -- -D warnings 2>&1 | grep -A 10 "対象ファイル名.rs"
      ```
5. **スニペット制約のチェック**:
    - `cargo-snippet` 展開時のパス不整合（`use crate::` の混入）を防ぐため、以下のコマンドでチェックを行ってください。
      ```bash
      cargo run --bin snippet_linter
      ```
6. **差分の確認**: `git diff` を使用して、意図しない変更がないか確認してください。関係のないファイルに変更があった場合は、ユーザーに報告・確認してください。
...
- **モジュール内のインポート**: `pub mod` の内部で `crate::...` を使ってインポートしないでください。
    - ファイルの先頭（`pub mod` の外）で `crate::...` を使ってインポートし、モジュール内では `super::...` を使って参照してください。
    - これは、`cargo-snippet` 等で 1 つのファイル（`main.rs`）に展開された際に、パスの不整合（提出用ファイルには `mylib` クレートが存在しない等）によるコンパイルエラーを防ぐためです。
    - 例：
      ```rust
      use crate::ab_group::ab_group::AbGroup; // 外側で crate:: からインポート
      pub mod potentialized_dsu {
          use super::AbGroup; // 内部では super:: を使用
          // ...
      }
      ```
...
テスト（特にランダムテスト）は、ファイル単位で **0.2秒以内** に終了することを確認してください。
