...
impl Problem {
    fn read<R: IProconReader>(mut r: R) -> Problem {
        ...
        let names = (0.._n).map(|_| r.read_line()).collect_vec();
        Problem { names }
    }
    fn solve(&self) -> Answer {
        let Problem { names } = self;
        let mut count_table: HashMap<String, i64> = HashMap::new();
        for name in names {
            if let Some(cnt) = count_table.get_mut(name) {
                *cnt += 1;
            } else {
                count_table.insert(name.clone(), 1);
            }
        }
        let ans = count_table.iter().max_by_key(|(_name, cnt)| **cnt).unwrap().0.clone();
        Answer { ans }
    }
    ...
    fn solve2(&self) -> Answer {
        ...
        let mut count_table: HashMap<String, i64> = HashMap::new();
        for name in names {
            *count_table.entry(name.clone()).or_insert(0) += 1;
        }
        let ans = count_table.iter().max_by_key(|(_name, cnt)| **cnt).unwrap().0.clone();
        Answer { ans }
    }
