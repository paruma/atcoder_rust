...
impl Problem {
    ...
    fn solve(&self) -> Answer {
        ...
        // next_len() を次数という。adjの代わりにを管理しておけばよかった。
        let ans = adj.iter().any(|next| next.len() == self.n - 1);
    ...
    fn solve1(&self) -> Answer {
        ...
        let ans = degree.contains(&(self.n - 1));
...
fn main() {
    Problem::read(ProconReader::new(stdin().lock())).solve1().print();
}
