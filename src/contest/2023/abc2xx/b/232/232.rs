...
impl Problem {
    ...
    fn caesar(s: &[u8], shift_cnt: u8) -> Vec<u8> {
        s.iter().map(|ch| (ch - b'a' + shift_cnt) % 26 + b'a').collect_vec()
    }

    fn solve(&self) -> Answer {
        let shift_cnt = (self.t[0] + 26 - self.s[0]) % 26;
        let after_s = Self::caesar(&self.s, shift_cnt);
        let ans = after_s == self.t;
        Answer { ans }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_caesar() {
        assert_eq!(Problem::caesar("xyz".as_bytes(), 2), "zab".as_bytes());
    }
