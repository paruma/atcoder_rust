...
impl Problem {
    fn read<R: IProconReader>(mut r: R) -> Problem {
        let (n, x) = r.read_usize_2();
        let x = x - 1;
        let next_list = r.read_vec_usize().iter().map(|a| a - 1).collect_vec();
        Problem { n, x, next_list }
    }
    fn solve(&self) -> Answer {
        let mut current = self.x;
        let mut visited = vec![false; self.n];
        visited[current] = true;
        // self.x は self.next_list[self.x] に教えちゃう
        loop {
            let next = self.next_list[current];
            if !visited[next] {
                visited[next] = true;
                current = next;
            } else {
