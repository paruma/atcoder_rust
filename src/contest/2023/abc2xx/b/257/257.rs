...
struct Problem {
    ...
    n_pieces: usize,
    n_queries: usize,
    piece_pos_list: Vec<usize>,
    queries: Vec<usize>,
}

impl Problem {
    fn read<R: IProconReader>(mut r: R) -> Problem {
        let (n_squares, n_pieces, n_queries) = r.read_usize_3();
        let piece_pos_list = r.read_vec_usize().iter().map(|i| *i - 1).collect_vec();
        let queries = r.read_vec_usize().iter().map(|i| *i - 1).collect_vec();
        Problem { n_squares, n_pieces, n_queries, piece_pos_list, queries }
    }
    fn solve(&self) -> Answer {
        let Problem { n_squares, n_pieces, n_queries, piece_pos_list, queries } = self;
        let mut piece_pos_list = piece_pos_list.clone();

        for &piece_idx in queries {
            // query 番目のコマを動かす
            // 一番右にいる
            if piece_pos_list[piece_idx] == n_squares - 1 {
                continue;
            }
            // 右隣にコマがある
            if piece_idx != n_pieces - 1
                && piece_pos_list[piece_idx + 1] == piece_pos_list[piece_idx] + 1
            {
                continue;
            }
            piece_pos_list[piece_idx] += 1;
        }

        Answer { ans: piece_pos_list.clone() }
    }
}
...
impl Answer {
    fn print(&self) {
        println!("{}", self.ans.iter().map(|i| i + 1).join(" "));
    }
}
