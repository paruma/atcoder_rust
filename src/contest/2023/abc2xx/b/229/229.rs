...
impl Problem {
    fn read<R: IProconReader>(mut r: R) -> Problem {
        let ab = r
            .read_vec_str()
            .iter()
            .map(|x| x.as_bytes().iter().map(|&xi| (xi - b'0') as i64).collect_vec())
            .collect_vec();
        let a = ab[0].clone();
        let b = ab[1].clone();
        Problem { a, b }
    ...
    fn solve(&self) -> Answer {
        let is_hard =
            Iterator::zip(self.a.iter().rev(), self.b.iter().rev()).any(|(ai, bi)| ai + bi >= 10);
        let ans = if is_hard { "Hard" } else { "Easy" };
