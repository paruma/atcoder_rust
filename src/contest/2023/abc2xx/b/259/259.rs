...
impl Problem {
    ...
    fn solve(&self) -> Answer {
        let x = self.x;
        let y = self.y;
        let rad = self.degree / 360.0 * 2.0 * PI;
        let ans_x = x * f64::cos(rad) - y * f64::sin(rad);
        let ans_y = y * f64::cos(rad) + x * f64::sin(rad);
