...
impl Problem {
    fn read<R: IProconReader>(mut r: R) -> Problem {
        let (n_seq, n_query) = r.read_usize_2();
        let seq_list = (0..n_seq).map(|_| r.read_vec_i64()).collect_vec();
        let query_list = (0..n_query)
            .map(|_| {
                let (seq_idx, term_idx) = r.read_usize_2();
                let seq_idx = seq_idx - 1;
                // term_idx は1オリジンのまま (0番目の配列のサイズが入っているため)
                Query { seq_idx, term_idx }
            })
            .collect_vec();
        Problem { n_seq, n_query, seq_list, query_list }
    }
    fn solve(&self) -> Answer {
        let Problem { n_seq, n_query, seq_list, query_list } = self;
        let ans = query_list.iter().map(|q| seq_list[q.seq_idx][q.term_idx]).collect_vec();
        Answer { ans }
    }
}
