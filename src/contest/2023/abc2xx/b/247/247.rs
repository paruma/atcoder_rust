...
impl Problem {
    fn read<R: IProconReader>(mut r: R) -> Problem {
        ...
        let names = (0..n)
            .map(|_| {
                let x = r.read_vec_str();
                Name { family: x[0].clone(), given: x[1].clone() }
            })
            .collect_vec();
        Problem { names }
    }
    fn solve(&self) -> Answer {
        ...
        for name in names {
            *name_count.entry(name.family.clone()).or_insert(0) += 1;
            if name.family != name.given {
                *name_count.entry(name.given.clone()).or_insert(0) += 1;
            }
        }

        let ans =
            names.iter().all(|name| name_count[&name.family] == 1 || name_count[&name.given] == 1);
