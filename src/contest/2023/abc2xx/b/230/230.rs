...
impl Problem {
    ...
    fn is_matching(s: &[u8], pattern: [u8; 3]) -> bool {
        s.iter().enumerate().all(|(i, &ch)| ch == pattern[i % 3])
    }
    ...
    fn solve(&self) -> Answer {
        ...
        let ans = Self::is_matching(&self.s, [b'o', b'x', b'x'])
            || Self::is_matching(&self.s, [b'x', b'x', b'o'])
            || Self::is_matching(&self.s, [b'x', b'o', b'x']);
        Answer { ans }
    }
}
