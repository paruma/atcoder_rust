...
fn is_valid_time(hour: i64, minute: i64) -> bool {
    0 <= hour && hour < 24 && 0 <= minute && minute < 60
}

fn is_confusing_time(hour: i64, minute: i64) -> bool {
    let incorrect_hour = (hour / 10) * 10 + (minute / 10); // hour と minute の10の位
    let incorrect_minute = (hour % 10) * 10 + (minute % 10); // hour と minute の10の位
    is_valid_time(incorrect_hour, incorrect_minute)
}

impl Problem {
    fn read<R: IProconReader>(mut r: R) -> Problem {
        let (current_hour, current_minute) = r.read_i64_2();
        Problem { current_hour, current_minute }
    }
    fn solve(&self) -> Answer {
        let Problem { current_hour, current_minute } = self;
        let (ans_hour, ans_minute) = (0..60 * 24)
            .map(|delta_minute| {
                let current_total_minute = current_hour * 60 + current_minute;
                let next_total_minute = current_total_minute + delta_minute;
                let next_hour = (next_total_minute / 60) % 24;
                let next_minute = next_total_minute % 60;
                (next_hour, next_minute)
            })
            .find(|(hour, minute)| is_confusing_time(*hour, *minute))
            .unwrap();
        Answer { ans_hour, ans_minute }
    }
}
...
impl Answer {
    fn print(&self) {
        println!("{} {}", self.ans_hour, self.ans_minute);
    }
}
