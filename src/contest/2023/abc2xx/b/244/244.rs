...
impl Problem {
    ...
    fn solve(&self) -> Answer {
        let t = &self.t;
        let diff = [Pos::new(1, 0), Pos::new(0, -1), Pos::new(-1, 0), Pos::new(0, 1)];
        struct State {
            pos: Pos<i64>,
            dir: usize, // 0: 東, 1: 南, 2: 西, 3: 北
        }
        let state =
            t.iter().fold(State { pos: Pos::new(0, 0), dir: 0 }, |acc: State, action: &u8| {
                match *action {
                    b'S' => {
                        // 真っ直ぐ進む
                        State { pos: acc.pos + diff[acc.dir], dir: acc.dir }
                    }
                    b'R' => {
                        // 右に曲がる
                        State { pos: acc.pos, dir: (acc.dir + 1) % 4 }
                    }
                    _ => panic!(),
                }
            });
