...
impl Problem {
    ...
    fn shift(s: &Vec<u8>, n_shift: usize) -> Vec<u8> {
        // 左シフト
        (0..s.len()).map(|i| s[(i + n_shift) % s.len()]).collect_vec()
    }

    fn solve(&self) -> Answer {
        let n = self.s.len();
        let cand = (0..n).map(|n_shift| Self::shift(&self.s, n_shift)).sorted().collect_vec();
        Answer { s_min: cand.first().unwrap().clone(), s_max: cand.last().unwrap().clone() }
    }
}
...
impl Answer {
    fn print(&self) {
        println!("{}", String::from_utf8(self.s_min.clone()).unwrap());
        println!("{}", String::from_utf8(self.s_max.clone()).unwrap());
    }
}
