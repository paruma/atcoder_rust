...
impl Problem {
    fn read<R: IProconReader>(mut r: R) -> Problem {
        ...
        let table = (0..n).map(|_| r.read_bytes()).collect_vec();
        Problem { n, table }
    }
    fn solve(&self) -> Answer {
        let n = self.n;
        let table = &self.table;
        let is_incorrect = iproduct!(0..n, 0..n).any(|(i, j)| {
            table[i][j] == b'W' && table[j][i] != b'L'
                || table[i][j] == b'L' && table[j][i] != b'W'
                || table[i][j] == b'D' && table[j][i] != b'D'
        });
        Answer { is_correct: !is_incorrect }
    }
}
...
impl Answer {
    fn print(&self) {
        let msg = if self.is_correct { "correct" } else { "incorrect" };
        println!("{}", msg);
    }
}
