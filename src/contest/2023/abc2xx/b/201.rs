...
impl Problem {
    fn read<R: IProconReader>(mut r: R) -> Problem {
        let n_mountains = r.read_usize_1();
        let mountains = (0..n_mountains)
            .map(|_| {
                let (name, height) = r.read_any_2::<String, i64>();
                Mountain { name, height }
            })
            .collect_vec();
        Problem { n_mountains, mountains }
    }
    fn solve(&self) -> Answer {
        let sorted_mountains =
            self.mountains.iter().sorted_by_key(|mountain| Reverse(mountain.height)).collect_vec();
        let ans = sorted_mountains[1].name.clone();
        Answer { ans }
    }
}
