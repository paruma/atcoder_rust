...
impl Problem {
    ...
    fn solve(&self) -> Answer {
        ...
        let goal_pos = self.x;
        let wall_pos = self.y;
        let hammer_pos = self.z;
        // スタートとゴールの間に壁がない→簡単
        // 壁がある場合
        // 壁の向こう側にハンマーがある場合は無理
        // そうでない場合はハンマーを取ってからゴールに向かう

        // x と y の間に z がある
        let is_within = |x: i64, y: i64, z: i64| {
            let begin_exclude = i64::min(x, y);
            let end_exclude = i64::max(x, y);
            begin_exclude < z && z < end_exclude
        };

        // スタートとゴールの間に壁がない
        let ans = if !is_within(start_pos, goal_pos, wall_pos) {
            Some(i64::abs(start_pos - goal_pos))
        } else {
            //壁がある→ハンマーを取りに行く
            // スタートとハンマーの間に壁がある→取りに行けない
            if is_within(start_pos, hammer_pos, wall_pos) {
                None
            } else {
                // ハンマーを取りに行く
                Some(i64::abs(start_pos - hammer_pos) + i64::abs(hammer_pos - goal_pos))
            }
        };
