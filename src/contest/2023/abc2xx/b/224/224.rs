...
impl Problem {
    fn read<R: IProconReader>(mut r: R) -> Problem {
        ...
        let grid = (0..h).map(|_| r.read_vec_i64()).collect_vec();
        Problem { h, w, grid }
    }
    fn solve(&self) -> Answer {
        let ans = Iterator::zip(
            (0..self.h).flat_map(|y1| ((y1 + 1)..self.h).map(move |y2| (y1, y2))),
            (0..self.w).flat_map(|x1| ((x1 + 1)..self.w).map(move |x2| (x1, x2))),
        )
        .all(|((y1, y2), (x1, x2))| {
            self.grid[y1][x1] + self.grid[y2][x2] <= self.grid[y2][x1] + self.grid[y1][x2]
        });
        Answer { ans }
    }
}
