...
impl Problem {
    fn read<R: IProconReader>(mut r: R) -> Problem {
        r.read_usize_2();
        let owned_lens = r.read_vec_i64();
        let plan_lens = r.read_vec_i64();
        Problem { owned_lens, plan_lens }
    }
    fn solve(&self) -> Answer {
        let Problem { owned_lens, plan_lens } = self;
        // 長さ→本数の表を作る
        let mut owned_len_cnt_table: HashMap<i64, i64> = HashMap::new();
        for &len in owned_lens {
            *owned_len_cnt_table.entry(len).or_insert(0) += 1;
        }

        let mut plan_len_cnt_table: HashMap<i64, i64> = HashMap::new();
        for &len in plan_lens {
            *plan_len_cnt_table.entry(len).or_insert(0) += 1;
        }

        for (len, plan_cnt) in &plan_len_cnt_table {
            // plan_cnt: 必要な量
            if plan_cnt > owned_len_cnt_table.get(len).unwrap_or(&0) {
