...
impl Problem {
    fn read<R: IProconReader>(mut r: R) -> Problem {
        let price = r.read_i64_1();
        Problem { price }
    }
    fn solve(&self) -> Answer {
        let coins = (2..=10).scanl(1, |acc, x| *acc * x).collect_vec();
        assert_eq!(coins[0], 1);
        assert_eq!(coins[1], 2);
        assert_eq!(coins[2], 6);
        ...
        let mut cnt = 0;
        for &coin_value in coins.iter().rev() {
            // 使うコインの枚数(100枚までしか使えない)
            let n_coins = i64::min(price / coin_value, 100);
            cnt += n_coins;
            price -= coin_value * n_coins;
            assert!(price >= 0);
        }
        let ans = cnt;
        Answer { ans }
