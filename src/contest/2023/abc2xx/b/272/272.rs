...
struct Problem {
    n_people: usize,
    n_parties: usize,
    attendance_info: Vec<HashSet<usize>>,
}

impl Problem {
    fn read<R: IProconReader>(mut r: R) -> Problem {
        let (n_people, n_parties) = r.read_usize_2();
        let attendance_info = (0..n_parties)
            .map(|_| {
                let row = r.read_vec_usize();
                row[1..].iter().map(|i| *i - 1).collect::<HashSet<_>>()
            })
            .collect_vec();
        Problem { n_people, n_parties, attendance_info }
    }
    fn solve(&self) -> Answer {
        let Problem { n_people, n_parties, attendance_info } = self;
        let ans = (0..*n_people).flat_map(|i| (i..*n_people).map(move |j| (i, j))).all(|(i, j)| {
            // i, j: 人
            // どこかのパーティで i と j が一緒だったかどうか
            attendance_info
                .iter()
                .any(|attendance_set| attendance_set.contains(&i) && attendance_set.contains(&j))
        });
        Answer { ans }
    }
}
...
impl Answer {
    fn print(&self) {
        let ans = if self.ans { "Yes" } else { "No" };
        println!("{}", ans);
    }
}
