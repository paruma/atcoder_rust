...
impl Problem {
    fn read<R: IProconReader>(mut r: R) -> Problem {
        let (display_height, display_width) = r.read_i64_2();
        let (image_height, image_width) = r.read_i64_2();
        Problem { display_height, display_width, image_height, image_width }
    }
    fn solve(&self) -> Answer {
        let Problem { display_height, display_width, image_height, image_width } = self;
        let v_score = min(image_width * display_height, image_height * display_width);
        let h_score = min(image_height * display_height, image_width * display_width);
        let ans = if v_score > h_score {
            "Vertical"
