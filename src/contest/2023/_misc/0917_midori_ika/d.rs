...
fn rev_vertex(n: usize, grid: &Vec<Vec<i64>>) -> Vec<Vec<i64>> {
    (0..n).map(|y| (0..n).map(|x| grid[n - y - 1][x]).collect_vec()).collect_vec()
}

fn rev_horizontal(n: usize, grid: &Vec<Vec<i64>>) -> Vec<Vec<i64>> {
    (0..n).map(|y| (0..n).map(|x| grid[y][n - x - 1]).collect_vec()).collect_vec()
}

fn rotate(n: usize, grid: &Vec<Vec<i64>>) -> Vec<Vec<i64>> {
    (0..n).map(|y| (0..n).map(|x| grid[n - x - 1][y]).collect_vec()).collect_vec()
}

fn transpose(n: usize, grid: &Vec<Vec<i64>>) -> Vec<Vec<i64>> {
    (0..n).map(|y| (0..n).map(|x| grid[x][y]).collect_vec()).collect_vec()
}

impl Problem {
    fn read<R: IProconReader>(mut r: R) -> Problem {
        let n = r.read_usize_1();
        let grid_s = (0..n).map(|_| r.read_vec_i64()).collect_vec();

        let grid_t = (0..n).map(|_| r.read_vec_i64()).collect_vec();
        Problem { n, grid_s, grid_t }
    }
    fn solve(&self) -> Answer {
        let n = self.n;
        let ans = iproduct!([0, 1], [0, 1], [0, 1, 2, 3]).any(
            |(cnt_rev_vertex, cnt_transpose, cnt_rotate)| {
                let mut grid = self.grid_s.clone();
                for _ in 0..cnt_rev_vertex {
                    grid = rev_vertex(n, &grid)
                }
                for _ in 0..cnt_transpose {
                    grid = transpose(n, &grid)
                }
