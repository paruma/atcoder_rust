...
impl Problem {
    ...
    fn solve0(&self) -> Answer {
        ...
        let ans = (0..n)
            .permutations(n)
            .map(|route| (0..n - 1).map(|i| route[i] + route[i + 1]).unique().count())
            .min()
            .unwrap() as i64;
...
mod tests {
    ...
    fn check(input: &str, expected: Answer) {
        let actual = Problem::read(ProconReader::new(input.as_bytes())).solve0();
