...
impl Problem {
    ...
    fn solve(&self) -> Answer {
        ...
        let mut dp = vec![HashMap::<Pos<i64>, i64>::new(); n + 1];

        dp[0].insert(Pos::new(0, 0), 0);

        for (i, dish) in dishes.iter().copied().enumerate() {
            let mut next_dp = HashMap::<Pos<i64>, i64>::new();
            for (k, v) in dp[i].iter() {
                // 選ぶ
                if k.x <= max.x && k.y <= max.y {
                    let next = *k + dish;
                    //next_dp.insert(next, *v + 1);

                    next_dp.entry(next).or_insert(0);
                    *next_dp.get_mut(&next).unwrap() = i64::max(next_dp[&next], *v + 1);
                }
                // 選ばない
                next_dp.entry(*k).or_insert(0);
                *next_dp.get_mut(k).unwrap() = i64::max(next_dp[k], *v);
            }
            dp[i + 1] = next_dp;
        }

        let ans = dp[n].values().copied().max().unwrap();
        Answer { ans }
    }
