...
impl Problem {
    ...
    fn solve(&self) -> Answer {
        ...
        let ans = (0..nv)
            .permutations(nv)
            .map(|perm| {
                // H に perm を作用させて新しくグラフを作る
                let new_h_edge_set = h_edge_set
                    .iter()
                    .copied()
                    .map(|e| Edge {
                        u: perm[e.u],
                ...
                let mut perm_inv = vec![1000; nv];
                for i in 0..nv {
                    perm_inv[perm[i]] = i;
                }

                let cost_sum = g_edge_set
                    .symmetric_difference(&new_h_edge_set)
                    .map(|e| self.costs[perm_inv[e.u]][perm_inv[e.v]])
                    .sum::<i64>()
                    / 2;
                cost_sum
            })
