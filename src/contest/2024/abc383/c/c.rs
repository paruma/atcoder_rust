...
impl Problem {
    ...
    fn solve(&self) -> Answer {
        ...
        for p in grid.all_pos_iter().filter(|p| *grid.at(*p) == 'H') {
            if self.d >= 1 {
                open.push(p);
            }
            *visited.at_mut(p) = true;
            *dist.at_mut(p) = 0;
        }

        while let Some(current) = open.pop() {
            for to in Pos::around4_pos_iter(current).filter(|p| grid.can_move(*p)) {
                if !visited.at(to) {
                    *visited.at_mut(to) = true;
                    *dist.at_mut(to) = dist.at(current) + 1;
                    if *dist.at(to) < self.d as i64 {
                        open.push(to);
                    }
                }
            }
