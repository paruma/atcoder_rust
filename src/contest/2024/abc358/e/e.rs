...
impl Problem {
    ...
    fn solve(&self) -> Answer {
        ...
        //let mut dp = vec![vec![vec![vec![Mint::new(0); c_max + 1]; k + 1]; 26]; 26];
        let mut dp = vec![vec![vec![vec![Mint::new(0); 26]; c_max + 1]; 26]; k + 1];
        ...
        for i in 0..k {
            for ch1 in 0..26 {
                for res in 1..=c_max {
                    ...
                    let tmp = (0..26).map(|ch| dp[i][ch1][res - 1][ch]).sum::<Mint>();

                    lg!(tmp);

                    for ch2 in 0..26 {
                        dp[i + 1][ch1][res][ch2] = tmp;
                    }
                }
            }
        }
        let ans = iproduct!(1..=k, 0..26)
            .map(|(i, ch2)| dp[i][0][cs[0]][ch2])
            .sum::<Mint>();
