...
fn main() {
    ...
    // 1, 2, / がある添字の場所
    let mut poss1 = FenwickTreeDenseMultiset::new(n);
    let mut poss2 = FenwickTreeDenseMultiset::new(n);
    let mut poss_slash = FenwickTreeDenseMultiset::new(n);
    ...
    for (l, r) in qs {
        let len = bin_search(-1, n as i64, |len| {
            ...
            if len == 0 {
                return poss_slash.count_in_range(l..=r) >= 1;
            }
            let Some(cur1) = poss1.nth_min_in_range(len - 1, l..=r) else {
                return false;
            };
            let Some(cur2) = poss_slash.nth_min_in_range(0, cur1 + 1..=r) else {
                return false;
            };
            let Some(_) = poss2.nth_min_in_range(len - 1, cur2 + 1..=r) else {
                return false;
            };
            true
        });
        let ans = if len == -1 { 0 } else { 2 * len + 1 };
        println!("{}", ans);
    }
}
