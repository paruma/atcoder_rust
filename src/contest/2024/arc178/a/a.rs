...
impl Problem {
    ...
    fn solve(&self) -> Answer {
        ...
        if xs.iter().copied().any(|x| x == 1 || x == n) {
            return Answer { ans: None };
        }

        let xs = xs.iter().copied().sorted().collect_vec();
        let mut set = (0..n).collect::<BTreeSet<usize>>();

        let mut xs_iter = xs.iter().copied().peekable();

        let mut ans: Vec<usize> = vec![];

        for i in 0..n {
            if Some(i) == xs_iter.peek().map(|x| *x - 1) {
                let v = *set.iter().nth(1).unwrap();
                ans.push(v); //2番目に小さいの
                set.remove(&v);
                xs_iter.next();
            } else {
    ...
    fn solve_naive(&self) -> Answer {
        ...
        //     .permutations(n)
        //     .filter(|perm| {
        //         // x in xs に対して (0..x) の順列を perm が含まないことを確認する
        //         xs.iter().copied().all(|x| {
        //             (0..perm.len() - x + 1).all(|begin| {
        //                 let end = begin + x;
        //                 let sub_perm = perm[begin..end].iter().copied().sorted().collect_vec();
        //                 sub_perm != (0..x).collect_vec()
        //             })
        //         })
        //     })
        //     .sorted()
