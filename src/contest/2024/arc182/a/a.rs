...
impl Problem {
    ...
    fn solve(&self) -> Answer {
        ...
        let mut left_is_ok = vec![true; nq]; // ←
        let mut right_is_ok = vec![true; nq]; // →

        {
            let mut max_seg = Segtree::<Max<i64>>::from(vec![0; n]);

            for (i, q) in qs.iter().enumerate() {
                if max_seg.prod(0..=q.p) > q.v {
                    left_is_ok[i] = false;
                }
                if max_seg.prod(q.p..n) > q.v {
                    right_is_ok[i] = false;
                }
                max_seg.set(q.p, max_seg.get(q.p).max(q.v));
            }
        }
        {
            let mut min_seg = Segtree::<Min<i64>>::from(vec![i64::MAX; n]);

            for (i, q) in qs.iter().enumerate().rev() {
                if min_seg.prod(0..=q.p) < q.v {
                    left_is_ok[i] = false;
                }
                if min_seg.prod(q.p..n) < q.v {
                    right_is_ok[i] = false;
                }
                min_seg.set(q.p, min_seg.get(q.p).min(q.v));
            }
        }

        //let ans_foreach = vec![Mint::new(1); nq];
        let ans_foreach = (0..nq)
            .map(|i| {
                let cnt = (left_is_ok[i] as i64) + (right_is_ok[i] as i64);
                Mint::new(cnt)
            })
            .collect_vec();
        let ans = ans_foreach.iter().product::<Mint>().val() as i64;
        Answer { ans }
    }
...
mod tests {
    ...
    fn test_with_naive() {
        let num_tests = 10000;
        let max_wrong_case = 100; // この件数間違いが見つかったら打ち切り
