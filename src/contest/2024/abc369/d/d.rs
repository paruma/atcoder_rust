...
impl Problem {
    ...
    fn solve(&self) -> Answer {
        let n = self.n;
        let xs = &self.xs;
        let mut dp: Vec<Vec<NegExtInt>> = vec![vec![NegInf; 2]; n + 1];
        // dp[i][r] = [0,i) で モンスターを倒した回数 mod 2 == r のときの経験値最大値
        ...
        for i in 0..n {
            // i番目選ぶ or 選ばない
            for r in 0..2 {
                let exp = if r == 0 { Fin(xs[i] * 2) } else { Fin(xs[i]) };
                let choose = dp[i][1 - r] + exp;
                let no_choose = dp[i][r];

                dp[i + 1][r] = NegExtInt::max(choose, no_choose);
            }
        }

        let ans = NegExtInt::max(dp[n][0], dp[n][1]).get_fin();
        Answer { ans }
    }
