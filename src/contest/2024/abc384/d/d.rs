...
impl Problem {
    ...
    fn solve(&self) -> Answer {
        let xs = &self.xs;
        let sum_1loop = xs.iter().copied().sum::<i64>();

        let remain = self.s % sum_1loop;

        let xs2 = chain(xs, xs).copied().collect_vec();

        let cumsum = prefix_sum(&xs2);

        for begin in 0..xs2.len() {
            // cumsum[end] - cumsum[begin] == remain となるような end が存在するか判定
            // cumsum[end] == cumsum[begin] + remain

            let exists = cumsum[begin..]
                .binary_search(&(cumsum[begin] + remain))
                .is_ok();
            if exists {
