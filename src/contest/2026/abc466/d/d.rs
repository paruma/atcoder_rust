...
fn main() {
    ...
    let mut rows = vec![BTreeSet::<usize>::new(); n];
    let mut cols = vec![BTreeSet::<usize>::new(); n];

    for (i, (r, c)) in rcs.iter().copied().enumerate() {
    ...
    let ans = rows.iter().map(|s| s.len()).sum::<usize>();
    println!("{}", ans);
}
