...
mod tests {
    ...
    fn solve_naive(xs: &[i64]) -> usize {
        xs.iter()
            .copied()
            .tuple_combinations()
            .filter(|(x1, x2)| x2 - x1 <= 10)
            .count()
    }
    ...
    fn process_one_test(rng: &mut SmallRng) -> bool {
        ...
        let mut xs = vec![rng.random_range(-100..100)];
        for i in 1..n {
            xs.push(xs[i - 1] + rng.random_range(1..30))
        }
        ...
        // ==== 間違っていたら報告をする ====
        if main_ans != naive_ans || asker.cnt_ask > 2 * n {
            // 問題を出力
            println!("{:?}", (n, xs));
