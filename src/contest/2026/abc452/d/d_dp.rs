fn main() {
    ...
    // dp[r][i] = #{ l in 0..r | s[l..r] は t[..i] を（連続とは限らない）部分列として含む }
    // 公式解説では、「t[i..] は含まない」（含むのは先頭ちょうどi文字まで）という制約を付けているが、
    // この実装では付けていない。
    ...
    for r in 1..=s.len() {
        for i in 1..=t.len() {
            dp[r][i] = if s[r - 1] == t[i - 1] {
                dp[r - 1][i - 1] + (i == 1) as i64
            } else {
                dp[r - 1][i]
            };
        }
    ...
    let ans =
        (s.len() * (s.len() + 1) / 2) as i64 - (0..=s.len()).map(|r| dp[r][t.len()]).sum::<i64>();
