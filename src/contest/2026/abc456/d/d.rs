fn main() {
    ...
    let n = xs.len();
    // dp[i][ch] = 最後の文字がchのときの場合の数
    let mut dp = vec![[Mint::new(0); 3]; n + 1];
    dp[0][0] = Mint::new(1);
    dp[0][1] = Mint::new(1);
    dp[0][2] = Mint::new(1);
    ...
    for i in 0..n {
        for j in 0..3 {
            let target_ch = b'a' + j as u8;
            if xs[i] == target_ch {
                // xs[i] は採用できない
                dp[i + 1][j] = dp[i][0] + dp[i][1] + dp[i][2] + Mint::new(1);
            } else {
                dp[i + 1][j] = dp[i][j];
            }
        }
    ...
    let ans: Mint = dp[n][0] + dp[n][1] + dp[n][2];
    println!("{}", ans);
}
#[cfg(test)]
mod tests {
