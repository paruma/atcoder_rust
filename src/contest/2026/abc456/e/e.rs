...
fn main() {
    ...
    for _ in 0..t {
        ...
        let id = |v: usize, d: usize| v * w + d;

        let mut adj = vec![vec![]; nv * w];

        // とどまる
        for v in 0..nv {
            for d in 0..w {
                let next_d = (d + 1) % w;
                if holy_table[v][d] == 'o' && holy_table[v][next_d] == 'o' {
                    adj[id(v, d)].push(id(v, next_d));
                }
            }
        }

        // 進む
        for (u, v) in es.iter().copied().flat_map(|(u, v)| [(u, v), (v, u)]) {
            for d in 0..w {
                let next_d = (d + 1) % w;
                if holy_table[u][d] == 'o' && holy_table[v][next_d] == 'o' {
                    adj[id(u, d)].push(id(v, next_d));
                }
            }
        }

        let ans = has_cycle_directed(&adj);
        print_yesno(ans);
    }
}
