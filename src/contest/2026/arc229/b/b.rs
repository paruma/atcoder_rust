...
fn solve(n: usize, xs: &[i64]) -> Option<i64> {
    // 7 4 みたいなのはだめ
    if xs.iter().copied().tuple_windows().any(|(x, y)| x < 2 * y) {
        return None;
    }
    ...
    loop {
        // lg!(&xs);
        // 右から見て最初の非0
        let Some(pos_non0) = xs.iter().rposition(|x| *x != 0) else {
            break;
        };
        let old_xs = xs.clone();
        ...
        if pos_non0 == n - 1 {
            ...
            let factor = old_xs[pos_non0]; // まとめて引く回数
            let mut cur = old_xs[pos_non0];
            cnt += factor;
            // lg!(factor);
            xs[pos_non0] = 0;
            for i in (0..pos_non0).rev() {
                // [2 * cur, 2 * cur + factor]
                if old_xs[i] <= 2 * cur + factor {
                    cur = old_xs[i];
                } else {
                    cur = 2 * cur + factor;
                }
