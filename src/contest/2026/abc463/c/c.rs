...
enum Event {
    Leave { height: i64, time: i64 },
    Out { index: usize, time: i64 },
}
...
fn main() {
    ...
    for &(h, l) in &hls {
        es.push(Event::Leave { height: h, time: l });
    }

    for (i, t) in ts.iter().copied().enumerate() {
        es.push(Event::Out { index: i, time: t })
    }

    es.sort_by_key(|e| match e {
        Event::Leave { height, time } => time * 2,
        Event::Out { index, time } => time * 2 + 1,
    });

    let mut h_bag = BTreeMultiSet::new();
    ...
    for e in es {
        match e {
            Event::Leave { height, time } => {
                h_bag.remove1(&height);
            }
            Event::Out { index, time } => {
                ans[index] = *h_bag.max().unwrap();
            }
        }
    }
    print_vec(&ans);
