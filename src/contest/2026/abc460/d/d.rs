...
fn main() {
        ...
        .map(|y| {
            (0..w)
                .map(|x| {
                    ...
                    if grid[p] == '.'
                        || Pos::around8_pos_iter(p)
                            .filter(|next| grid.is_within(*next))
                            .all(|next| grid[next] != '.')
                    {
                        '.'
                    } else {
    ...
    let init = (0..h)
        .flat_map(|y| (0..w).map(move |x| Pos::new(x as i64, y as i64)))
        .filter(|&p| grid[p] == '#')
        .collect_vec();
    let adj = |p: Pos| p.around8_pos_iter().filter(|&next| grid.is_within(next));

    let dist = bfs_arbitrary(bounds, adj, init);
