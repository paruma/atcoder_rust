...
fn main() {
    ...
    for _ in 0..t {
        ...
        let xys = xys
            .iter()
            .copied()
            .sorted_by_key(|(x, y)| Reverse(*x - *y))
            .collect_vec();

        let xs = xys.iter().copied().map(|(x, _)| x).collect_vec();
        let ys = xys.iter().copied().map(|(_, y)| y).collect_vec();

        let cum_xs = CumSum::new(&xs);
        let cum_ys = CumSum::new(&ys);

        let pivot = (0..n).min_by_key(|&i| (xs[i], (xs[i] - ys[i]))).unwrap();

        let ans = (0..=n)
            .map(|i| {
                // 上から i 枚はクーポンで買う
                let term1 = cum_ys.range_sum(0..i);
                let term2 = cum_xs.range_sum(i..n);
