// クエリを後ろから読んでほぼ愚直に。塗られているところはスキップすれば、O(マスの数+クエリの数) で処理できる
fn main() {
    input! {
        h: usize,
        w: usize,
        nq: usize,
        mut qs: [(PosYX1, char); nq],
    }

    qs.insert(0, (Pos::new(w as i64 - 1, h as i64 - 1), 'A'));

    let mut ans = vec![vec!['.'; w]; h];
    for (p, ch) in qs.iter().copied().rev() {
        for y in (0..=p.y).rev() {
            // すでに y 行目が塗られている場合は、そこでやめる
            if ans[Pos::new(p.x, y)] != '.' {
                break;
            }
            // y行目を右から左に見る
            for x in (0..=p.x).rev() {
                // 左から右に見ていってすでに塗られている場所があれば終わり
                if ans[Pos::new(x, y)] != '.' {
                    break;
                }
                ans[Pos::new(x, y)] = ch;
