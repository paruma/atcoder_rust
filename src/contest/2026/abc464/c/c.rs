...
fn main() {
    input! {
        n_birds: usize,
        n_days: usize,
        adbs: [(Usize1, Usize1, Usize1); n_birds],
    }

    // コンテスト中は BTreeMultiset を使った
    let mut colors = HashBag::<usize, FxBuildHasher>::default();

    for &(a, _, _) in &adbs {
        colors.insert(a);
    }

    let mut adbs_by_day = vec![vec![]; n_days];

    for &(a, d, b) in &adbs {
        adbs_by_day[d].push((a, d, b));
    }

    let mut ans = vec![usize::MAX; n_days];

    for day in 0..n_days {
        for &(a, d, b) in &adbs_by_day[day] {
            colors.remove(&a);
            colors.insert(b);
        }

        ans[day] = colors.set_len();
    }
    print_vec(&ans);
}
...
use {
    btree_multiset::*,
    hashbag::HashBag,
    rustc_hash::{FxBuildHasher, FxHashSet, FxHasher},
};
