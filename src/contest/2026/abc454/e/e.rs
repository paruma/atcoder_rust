...
fn solve(n: usize, banned: Pos) -> Option<Vec<char>> {
    ...
    while cur != goal {
        ...
        let next_dir = priority
            .iter()
            .copied()
            .find(|&dir| {
                let next = cur + DIR4_LIST[dir];
                visited.is_within(next) && !visited[next]
            })
            .unwrap();

        ans.push(dir_chars[next_dir]);
