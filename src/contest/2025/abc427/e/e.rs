...
fn main() {
    ...
    let init = grid.find_pos_of('T').unwrap();
    open.push(init);
    visited[init] = true;
    dist[init] = 0;

    while let Some(current) = open.pop() {
        for to in Pos::around4_pos_iter(current).filter(|x| grid.can_move(*x)) {
            if !visited[to] {
                visited[to] = true;
                dist[to] = i64::min(dist[to], dist[current] + 1);
                open.push(to);
            }
        }
    }
    ...
    let mut ans = i64::MAX;
    {
        ...
        for y in 0..h {
            ...
            let sub_ans = dist[y][x] + (w as i64 - init.x);
            dbg!(sub_ans);
...

    {
        ...
        for x in 0..w {
            ...
            let sub_ans = dist[y][x] + (h as i64 - init.y);
            dbg!(sub_ans);
