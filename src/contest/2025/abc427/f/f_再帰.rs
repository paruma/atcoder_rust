...
fn solve0(xs: &[i64], m: i64) -> (Vec<i64>, Vec<i64>) {
    fn rec(
        ...
        acc: i64,
        last_used: bool,
        acc_list_last_used: &mut Vec<i64>,
        acc_list_last_unused: &mut Vec<i64>,
        xs: &[i64],
        m: i64,
    ) {
        if i == xs.len() {
            if last_used {
                acc_list_last_used.push(acc);
            } else {
                acc_list_last_unused.push(acc);
            }
            return;
        }
    ...
    let mut acc_list_last_used = vec![];
    let mut acc_list_last_unused = vec![];
    rec(
        0,
        0,
        false,
        &mut acc_list_last_used,
        &mut acc_list_last_unused,
        xs,
        m,
    );
    (acc_list_last_unused, acc_list_last_used)
}
