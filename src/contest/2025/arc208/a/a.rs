...
fn main() {
    ...
    for _ in 0..t {
        ...
        let or = xs.iter().copied().fold(0, |acc, x| acc | x);
        let xor = xs.iter().copied().fold(0, |acc, x| acc ^ x);
        let g = xor ^ or;
...
mod tests {
    ...
    fn test_problem() {
        // n = 3 でやってみる
        let k = 5;
        let mut g = vec![vec![vec![0; k + 1]; k + 1]; k + 1];
        ...
        for i0 in 0..=k {
            for i1 in 0..=k {
                for i2 in 0..=k {
                    ...
                    // 0番目の山
                    for n0 in 0..i0 {
                        if i0 | i1 | i2 != n0 | i1 | i2 {
                            continue;
                        }
                        next_g.push(g[n0][i1][i2]);
                    }

                    // 1番目の山
                    for n1 in 0..i1 {
                        if i0 | i1 | i2 != i0 | n1 | i2 {
                            continue;
                        }
                        next_g.push(g[i0][n1][i2]);
                    }

                    // 2番目の山
                    for n2 in 0..i2 {
                        if i0 | i1 | i2 != i0 | i1 | n2 {
                            continue;
                        }
                        next_g.push(g[i0][i1][n2]);
                    }

                    g[i0][i1][i2] = mex(&next_g);
                }
            }
        }
        // for i0 in 0..=k {
    ...
    fn test_problem2() {
        // n = 4 でやってみる
        let k = 4;
        let mut g = vec![vec![vec![vec![0; k + 1]; k + 1]; k + 1]; k + 1];
            ...
            for i1 in 0..=k {
                for i2 in 0..=k {
                    for i3 in 0..=k {
                        ...
                        // 0番目の山
                        for n0 in 0..i0 {
                            next_g.push(g[n0][i1][i2][i3]);
                        }

                        // 1番目の山
                        for n1 in 0..i1 {
                            next_g.push(g[i0][n1][i2][i3]);
                        }

                        // 2番目の山
                        for n2 in 0..i2 {
                            next_g.push(g[i0][i1][n2][i3]);
                        }
                        // 3番目の山
                        for n3 in 0..i3 {
                            next_g.push(g[i0][i1][i2][n3]);
                        }

                        g[i0][i1][i2][i3] = mex(&next_g);
                    }
                }
            }
        }
