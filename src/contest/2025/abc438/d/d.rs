...
fn main() {
    ...
    let psum_xs = prefix_sum(&xs);
    let psum_ys = prefix_sum(&ys);
    let psum_zs = prefix_sum(&zs);
    ...
    let ans = (2..n)
        .map(|j| {
            let sa_m_sb = seg.prod(1..j);
            sa_m_sb - psum_xs[0] + psum_ys[j] + (psum_zs[n] - psum_zs[j])
        })
        .max()
        .unwrap();
