fn main() {
    ...
    let cum_xs = prefix_sum(&xs);
    let i_cum_xs = (0..=n).map(|i| (i as i64) * cum_xs[i]).collect_vec();
    let cum_cum_xs = CumSum::new(&cum_xs);
    let cum_i_cum_xs = CumSum::new(&i_cum_xs);
    ...
    let ans: Vec<i64> = (0..q)
        .map(|_| {
            ...
            // [L+1, R+1]
            let term1 = cum_i_cum_xs.range_sum(l + 1..=r + 1)
                - (l as i64) * cum_cum_xs.range_sum(l + 1..=r + 1);
            let term2 =
                (r as i64 + 1) * cum_cum_xs.range_sum(l..=r) - cum_i_cum_xs.range_sum(l..=r);
