...
fn main() {
    ...
    // let ys_max = ys.iter().copied().max().unwrap();
    // let max = usize::max(xs_max, ys_max);
    let max = {
        let xs_max = xs.iter().copied().max().unwrap();
        let ys_max = ys.iter().copied().max().unwrap();
        usize::max(xs_max, ys_max)
    };
    let cnt_x = xs.iter().copied().counts_vec(max + 1);
    let cnt_y = ys.iter().copied().counts_vec(max + 1);

    let comb: Comb<Mint> = Comb::new(2 * max);

    let fs = (0..=max)
        .map(|a| Mint::new(cnt_x[a]) * comb.factorial(a))
        .collect_vec();
    let gs = (0..=max)
        .map(|b| Mint::new(cnt_y[b]) * comb.inv_factorial(b))
        .collect_vec();
    let hs = (0..=max).map(|c| comb.inv_factorial(c)).collect_vec();

    let ghs = ac_library::convolution(&gs, &hs);
    // 配列外参照があり得る。

    let ans: i64 = (0..=max).map(|c| fs[c] * ghs[c]).sum::<Mint>().val() as i64;
    println!("{}", ans);
}
