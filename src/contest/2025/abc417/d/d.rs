...
fn main() {
    ...
    let mut dp = vec![vec![usize::MAX; k]; n + 1];
    ...
    for j in (0..n).rev() {
        ...
        for i in 0..k {
            if i <= present.price {
                dp[j][i] = dp[j + 1][i + present.up];
            } else {
                dp[j][i] = dp[j + 1][i.saturating_sub(present.down)];
            }
        }
    }
    ...
    // k未満になるまで down し続ける。
    let downs = ps.iter().copied().map(|x| x.down as i64).collect_vec();
    let downs_cumsum = CumSum::new(&downs);
    ...
    let ans: Vec<i64> = xs
        ...
        .map(|x| {
            // k 未満になるまで食べる。
            let cnt_consume = bin_search((n + 1) as i64, -1, |i| {
                x - downs_cumsum.range_sum(0..(i as usize)) < k as i64
            }) as usize;
            if cnt_consume == n + 1 {
                x - downs_cumsum.range_sum(0..n)
            } else {
                let start_idx = cnt_consume as usize;
                let start_tension = x - downs_cumsum.range_sum(0..start_idx);

                dp[start_idx][start_tension as usize] as i64
            }
        })
        .collect_vec();
    print_vec(&ans);
