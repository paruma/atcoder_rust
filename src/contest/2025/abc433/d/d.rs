...
pub fn keta(x: i64, base: i64) -> i64 {
    let mut x = x;
    let mut cnt = 0;
    while x > 0 {
        cnt += 1;
...
fn main() {
    ...
    // cnts[k] には xs[i] * 10^k mod m を保持する
    let mut cnts: Vec<HashBag<i64>> = vec![HashBag::new(); 11];

    for k in 1..=10 {
        let pow10 = 10i64.pow(k as u32) % m;
    ...
    let ans: i64 = xs
        ...
        .map(|x| {
            ...
            let target = (m - (x % m)) % m;
            cnts[d as usize].contains(&target) as i64
        })
        .sum::<i64>();
    println!("{}", ans);
}
