fn main() {
    ...
    let is_within = |p: Pos| 0 <= p.x && p.x < w as i64 && 0 <= p.y && p.y < h as i64;

    let mut que = vec![];
    let mut visited = vec![vec![false; w]; h];

    for p in iproduct!(0..h, 0..w).map(|(y, x)| Pos::new(x as i64, y as i64)) {
        if grid[p] == '#' {
            que.push(p);
            visited[p] = true;
        }
    ...
    for t in 0.. {
        ...
        for p in &que {
            for next in Pos::around4_pos_iter(*p) {
                if !is_within(next) || visited[next] {
                    continue;
                }
                // next の周りのマスの黒の数
                let cnt = Pos::around4_pos_iter(next)
                    .filter(|nn| is_within(*nn) && grid[*nn] == '#')
                    .count();
                if cnt == 1 {
                    next_que.push(next);
                    visited[next] = true;
    ...
    let ans: i64 = grid.iter().flatten().filter(|ch| **ch == '#').count() as i64;
    println!("{}", ans);
}
