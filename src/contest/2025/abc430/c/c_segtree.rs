...
fn main() {
    ...
    let a_seg = Segtree::<Additive<i64>>::from(inda);
    let b_seg = Segtree::<Additive<i64>>::from(indb);

    let ans: usize = (0..n)
        .map(|r| {
            // [l, r] で 'a' が a個以上となる l の最大値
            let la = {
                let la_p1 = a_seg.min_left(r + 1, |sum| *sum < a);
                if la_p1 == 0 {
                    return 0;
                }
                la_p1 - 1
            ...
            let lb = b_seg.min_left(r + 1, |sum| *sum < b);
