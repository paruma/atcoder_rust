fn check(grid: &[Vec<u8>], h: usize, w: usize) -> bool {
    iproduct!(0..h - 1, 0..w - 1).all(|(y, x)| {
        grid[y][x] == b'.'
            || grid[y + 1][x] == b'.'
            || grid[y][x + 1] == b'.'
            || grid[y + 1][x + 1] == b'.'
...
fn solve(grid: &[Vec<u8>], h: usize, w: usize) -> usize {
    if h == 2 && w == 2 && grid.iter().flatten().all(|x| *x == b'#') {
        return 1;
    }
    let grid_origin = grid.to_vec();
        ...
        .find(|cnt| {
            iproduct!(1..h - 1, 1..w - 1)
                ...
                .any(|pattern| {
                    ...
                    // grid が条件を満たしているか判定
                    let is_ok = check(&grid, h, w);

                    // 戻す
                    for &p in &pattern {
                        grid[p] = grid_origin[p];
                    }
