...
pub mod range_chmin_chmax_affine_range_min_max {
        ...
        T: Copy + Ord + From<i64> + Bounded + Add<Output = T> + Mul<Output = T> + std::fmt::Debug,
    {
        ...
        fn mapping(f: &Self::F, x: &<Self::M as Monoid>::S) -> <Self::M as Monoid>::S {
            ...
            let (mut transformed_min, mut transformed_max) = if f.mul_val >= T::from(0) {
                (
                    min_val * f.mul_val + f.add_val,
                    max_val * f.mul_val + f.add_val,
                )
            } else {
                (
                    max_val * f.mul_val + f.add_val,
            ...
            MinMaxResult {
                min_val: transformed_min,
                max_val: transformed_max,
            }
        }
    }
        ...
        T: Copy + Ord + From<i64> + Bounded + Add<Output = T> + Mul<Output = T> + std::fmt::Debug,
    {
        ...
        pub fn apply_range_affine<R>(&mut self, range: R, mul: T, add: T)
        where
            R: RangeBounds<usize>,
        {
            self.segtree
...
pub mod test_range_chmin_chmax_affine_range_min_max {
    use super::range_chmin_chmax_affine_range_min_max::{
        MinMaxResult, RangeChminChmaxAffineRangeMinMaxSegtree,
    };
    ...
    fn test_new_and_get() {
        ...
        let mut segtree = RangeChminChmaxAffineRangeMinMaxSegtree::<i64>::new(&xs);
    ...
    #[test]
    fn test_apply_range_affine() {
        let xs = vec![10, 20, 30, 40, 50];
        let mut segtree = RangeChminChmaxAffineRangeMinMaxSegtree::<i64>::new(&xs);
        segtree.apply_range_affine(1..4, 2, 3); // x -> 2x + 3
        assert_eq!(
            segtree.to_vec(),
        ...
        let xs = vec![10, 20, 30, 40, 50];
        let mut segtree = RangeChminChmaxAffineRangeMinMaxSegtree::<i64>::new(&xs);
        segtree.apply_range_affine(1..4, -1, 100); // x -> -x + 100
        assert_eq!(
            segtree.to_vec(),
    ...
    fn test_apply_range_update() {
        let xs = vec![10, 20, 30, 40, 50];
        let mut segtree = RangeChminChmaxAffineRangeMinMaxSegtree::<i64>::new(&xs);
        segtree.apply_range_update(1..4, 100);
    ...
    fn test_chmin_chmax_affine() {
        ...
        segtree.apply_range_chmin(0..5, 35); // [10, 20, 30, 35, 35]
        segtree.apply_range_chmax(0..5, 15); // [15, 20, 30, 35, 35]
        segtree.apply_range_affine(0..5, 2, 3); // x -> 2x + 3
        ...
        for _ in 0..100 {
            ...
            for _ in 0..100 {
                ...
                match op_type {
                    ...
                    4 => {
                        // apply_range_affine(range, mul, add)
                        let l = rng.random_range(0..=n);
                        let r = rng.random_range(l..=n);
                        ...
                        segtree.apply_range_affine(l..r, mul, add);
                    }
                    5 => {
                        // range_minmax(range)
                        let l = rng.random_range(0..=n);
                        ...
                        if l < r {
                            ...
                            assert_eq!(
                                result.min_val, expected_min,
                                "range_min({}..{}) failed",
                                l, r
                            );
                            assert_eq!(
                                result.max_val, expected_max,
                                "range_max({}..{}) failed",
                                l, r
                            );
                        } else {
                            let result = segtree.range_minmax(l..r);
                            assert_eq!(result.min_val, i64::MAX);
                            assert_eq!(result.max_val, i64::MIN);
                        }
                    }
                    6 => {
                        // all_minmax
                        ...
                        assert_eq!(result.min_val, expected_min, "all_min() failed");
                        assert_eq!(result.max_val, expected_max, "all_max() failed");
                    }
                    _ => unreachable!(),
                }
                assert_eq!(
