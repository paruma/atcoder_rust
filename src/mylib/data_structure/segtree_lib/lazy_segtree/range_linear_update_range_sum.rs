...
pub mod test_range_linear_update_range_sum {
    ...
    #[test]
    fn test_range_linear_update_with_diff() {
        ...
        // i=2: 10 + 2 * (2 - 1) = 12
        // i=3: 10 + 2 * (3 - 1) = 14
        segtree.range_linear_update(1..4, 10, 2);
        ...
        // i=3: 1 + 1 * (3 - 0) = 4
        // i=4: 1 + 1 * (4 - 0) = 5
        segtree.range_linear_update(0..5, 1, 1);
        assert_eq!(segtree.to_vec(), vec![1, 2, 3, 4, 5]);
    }

    #[test]
    fn test_range_update_with_zero_diff() {
        let xs = vec![10, 20, 30, 40, 50];
        let mut segtree = RangeLinearUpdateRangeSumSegtree::<i64>::from_slice(&xs);
        segtree.range_linear_update(1..4, 5, 0);
        assert_eq!(segtree.to_vec(), vec![10, 5, 5, 5, 50]);
        segtree.range_linear_update(0..3, -10, 0);
    ...
    fn test_to_vec() {
        ...
        segtree.range_linear_update(1..4, 10, 0);
    ...
    fn test_modint() {
        ...
        segtree.range_linear_update(0..3, Mint::new(1), Mint::new(0));
        assert_eq!(
            segtree.to_vec(),
        ...
        for _ in 0..100 {
            ...
            for _ in 0..100 {
                ...
                match op_type {
                    ...
                    1 => {
                        // range_linear_update(range, init, diff)
                        ...
                        segtree.range_linear_update(l..r, init, diff);
                    }
                    2 => {
                        // get(p)
                        let p = rng.random_range(0..n);
