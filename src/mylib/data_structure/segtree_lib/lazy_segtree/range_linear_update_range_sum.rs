...
pub mod range_linear_update_range_sum {
    ...
    where
        T: Copy + Add<Output = T> + Mul<Output = T> + Div<Output = T> + Sub<Output = T> + From<i64>,
    {
        pub fn new(xs: &[T]) -> RangeLinearUpdateRangeSumSegtree<T> {
            let xs = xs
                .iter()
                .copied()
                .enumerate()
...
pub mod test_range_linear_update_range_sum {
    ...
    fn test_modint() {
        ...
        let mut segtree = RangeLinearUpdateRangeSumSegtree::<Mint>::new(&xs);
    ...
    fn test_random_linear_update() {
        ...
        for _ in 0..100 {
            ...
            let mut segtree = RangeLinearUpdateRangeSumSegtree::<i64>::new(&naive_vec);
