...
pub mod monoid_union_find {
    ...
    impl<M: Monoid> MonoidUnionFind<M> {
        ...
        pub fn unite(&mut self, x: usize, y: usize) -> bool {
            ...
            let count = smaller_root.count + larger_root.count;
            let prod = M::binary_operation(&smaller_root.prod, &larger_root.prod);

            // larger_root に smaller_root をくっつける
            self.nodes[smaller_root_idx] = Node::NonRoot(NonRootInfo {
                parent_index: larger_root_idx,
            });
            self.nodes[larger_root_idx] = Node::Root(RootInfo { count, prod });
