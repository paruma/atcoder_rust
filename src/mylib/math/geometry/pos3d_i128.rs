    ...
    impl Pos3DI128 {
        ...
        pub fn to_index_1d(self, width: usize, height: usize) -> usize {
            assert!(
                ...
                "Pos3DI128::to_index_1d: x, y, z は 0 以上である必要があります。pos: ({}, {}, {})",
                self.x,
                self.y,
                self.z
            );
            assert!(
                ...
                "Pos3DI128::to_index_1d: x は width 未満である必要があります。x: {}, width: {}",
                self.x,
                width
            );
            assert!(
                (self.y as usize) < height,
                "Pos3DI128::to_index_1d: y は height 未満である必要があります。y: {}, height: {}",
                self.y,
                height
            );
        ...
        pub fn from_index_1d(index: usize, width: usize, height: usize) -> Pos3DI128 {
            let z = index / (width * height);
            let rem = index % (width * height);
            let y = rem / width;
            let x = rem % width;
            Pos3DI128::new(x as i128, y as i128, z as i128)
        }
...
#[snippet(prefix = "use vec_vec_vec_at_i128::*;")]
pub mod vec_vec_vec_at_i128 {
    use std::ops::{Index, IndexMut};

    use super::pos3d_i128::*;
    use easy_ext::ext;

    #[ext(ExtVecVecVecI128)]
    impl<T> Vec<Vec<Vec<T>>> {
        ...
        pub fn is_within(&self, pos: Pos3DI128) -> bool {
            (0..self.width() as i128).contains(&pos.x)
                && (0..self.height() as i128).contains(&pos.y)
                && (0..self.depth() as i128).contains(&pos.z)
        }
    }

    impl<T> Index<Pos3DI128> for Vec<Vec<Vec<T>>> {
        type Output = T;

        fn index(&self, index: Pos3DI128) -> &Self::Output {
    ...
    impl<T> IndexMut<Pos3DI128> for Vec<Vec<Vec<T>>> {
        fn index_mut(&mut self, index: Pos3DI128) -> &mut Self::Output {
            if cfg!(debug_assertions) && !self.is_within(index) {
                panic!(
                    "index out of bounds: the size (w, h, d) is ({}, {}, {}) but the index (x, y, z) is ({}, {}, {})",
                    self.width(),
...
#[snippet(prefix = "use pos3d_i128_ix::*;")]
pub mod pos3d_i128_ix {
    use super::Ix;
    use super::pos3d_i128::Pos3DI128;

    impl Ix for Pos3DI128 {
        fn range((min, max): (Self, Self)) -> impl Iterator<Item = Self> {
            (min.z..=max.z).flat_map(move |z| {
                (min.y..=max.y)
                    .flat_map(move |y| (min.x..=max.x).map(move |x| Pos3DI128::new(x, y, z)))
            })
        }
        ...
        fn from_index((min, max): (Self, Self), index: usize) -> Self {
            ...
            let width = (max.x - min.x + 1) as usize;
            let height = (max.y - min.y + 1) as usize;
            let dz = (index / (width * height)) as i128;
            let rem = index % (width * height);
            let dy = (rem / width) as i128;
            let dx = (rem % width) as i128;
            Pos3DI128::new(min.x + dx, min.y + dy, min.z + dz)
        }
...
mod tests_pos3d_i128 {
    ...
    #[test]
    fn test_pos3d_i128_index_1d() {
        let width = 10;
        let height = 5;
        let p = Pos3DI128::new(2, 3, 1); // z=1, y=3, x=2 -> 1*(10*5) + 3*10 + 2 = 50 + 30 + 2 = 82
        assert_eq!(p.to_index_1d(width, height), 82);
        assert_eq!(Pos3DI128::from_index_1d(82, width, height), p);
    }

    #[test]
    #[should_panic(expected = "x, y, z は 0 以上である必要があります")]
    fn test_pos3d_i128_index_1d_panic_negative() {
        Pos3DI128::new(-1, 0, 0).to_index_1d(10, 5);
    }

    #[test]
    #[should_panic(expected = "x は width 未満である必要があります")]
    fn test_pos3d_i128_index_1d_panic_width() {
        Pos3DI128::new(10, 0, 0).to_index_1d(10, 5);
    }

    #[test]
    #[should_panic(expected = "y は height 未満である必要があります")]
    fn test_pos3d_i128_index_1d_panic_height() {
        Pos3DI128::new(0, 5, 0).to_index_1d(10, 5);
    }
}

#[cfg(test)]
mod tests_vec_vec_vec_at_i128 {
    use super::pos3d_i128::*;
    use super::vec_vec_vec_at_i128::ExtVecVecVecI128;

    #[test]
    #[allow(clippy::useless_vec)]
    fn test_vec_vec_vec_at_i128() {
        ...
        // z=0: [[1, 2], [3, 4]]
        // z=1: [[5, 6], [7, 8]]
        assert_eq!(xsss[Pos3DI128::new(1, 0, 1)], 6);
        xsss[Pos3DI128::new(1, 0, 1)] = 60;
    ...
    #[allow(clippy::useless_vec)]
    fn test_vec_vec_vec_at_i128_panic_z() {
        let xsss = vec![vec![vec![1, 2], vec![3, 4]], vec![vec![5, 6], vec![7, 8]]];
        let _ = xsss[Pos3DI128::new(0, 0, 2)];
    }

    #[test]
    #[should_panic(expected = "index out of bounds")]
    #[allow(clippy::useless_vec)]
    fn test_vec_vec_vec_at_i128_panic_y() {
        let xsss = vec![vec![vec![1, 2], vec![3, 4]], vec![vec![5, 6], vec![7, 8]]];
        let _ = xsss[Pos3DI128::new(0, 2, 0)];
    }

    #[test]
    #[should_panic(expected = "index out of bounds")]
    #[allow(clippy::useless_vec)]
    fn test_vec_vec_vec_at_i128_panic_x() {
        let xsss = vec![vec![vec![1, 2], vec![3, 4]], vec![vec![5, 6], vec![7, 8]]];
        let _ = xsss[Pos3DI128::new(2, 0, 0)];
    }

    #[test]
    #[should_panic(expected = "index out of bounds")]
    #[allow(clippy::useless_vec)]
    fn test_vec_vec_vec_at_i128_panic_neg() {
        let xsss = vec![vec![vec![1, 2], vec![3, 4]], vec![vec![5, 6], vec![7, 8]]];
        let _ = xsss[Pos3DI128::new(-1, 0, 0)];
    }

    #[test]
    fn test_vec_vec_vec_at_i128_empty() {
...
#[cfg(test)]
mod tests_pos3d_i128_ix {
    use super::Ix;
    use super::pos3d_i128::*;

    #[test]
    fn test_pos3d_i128_ix() {
        let min = Pos3DI128::new(1, 1, 1);
        let max = Pos3DI128::new(2, 2, 2);
        let bounds = (min, max);
        assert_eq!(Pos3DI128::range_size(bounds), 8);

        let vec: Vec<_> = Pos3DI128::range(bounds).collect();
        assert_eq!(vec.len(), 8);
        assert_eq!(vec[0], Pos3DI128::new(1, 1, 1));
        assert_eq!(vec[1], Pos3DI128::new(2, 1, 1));
        assert_eq!(vec[2], Pos3DI128::new(1, 2, 1));
        assert_eq!(vec[3], Pos3DI128::new(2, 2, 1));
        assert_eq!(vec[4], Pos3DI128::new(1, 1, 2));
        assert_eq!(vec[7], Pos3DI128::new(2, 2, 2));

        assert_eq!(Pos3DI128::to_index(bounds, Pos3DI128::new(1, 1, 1)), 0);
        assert_eq!(Pos3DI128::to_index(bounds, Pos3DI128::new(2, 2, 2)), 7);
        assert_eq!(Pos3DI128::from_index(bounds, 0), Pos3DI128::new(1, 1, 1));
        assert_eq!(Pos3DI128::from_index(bounds, 7), Pos3DI128::new(2, 2, 2));

        assert!(Pos3DI128::in_range(bounds, Pos3DI128::new(1, 1, 1)));
        assert!(Pos3DI128::in_range(bounds, Pos3DI128::new(2, 2, 2)));
        assert!(!Pos3DI128::in_range(bounds, Pos3DI128::new(0, 1, 1)));
        assert!(!Pos3DI128::in_range(bounds, Pos3DI128::new(3, 1, 1)));
        assert!(!Pos3DI128::in_range(bounds, Pos3DI128::new(1, 0, 1)));
        assert!(!Pos3DI128::in_range(bounds, Pos3DI128::new(1, 3, 1)));
        assert!(!Pos3DI128::in_range(bounds, Pos3DI128::new(1, 1, 0)));
        assert!(!Pos3DI128::in_range(bounds, Pos3DI128::new(1, 1, 3)));
    }

    #[test]
    #[should_panic(expected = "index out of bounds")]
    fn test_pos3d_i128_ix_panic() {
        let min = Pos3DI128::new(1, 1, 1);
        let max = Pos3DI128::new(2, 2, 2);
        let bounds = (min, max);
        Pos3DI128::to_index(bounds, Pos3DI128::new(0, 0, 0));
    }

    #[test]
    #[should_panic(expected = "index out of range")]
    fn test_pos3d_i128_ix_from_index_panic() {
        let min = Pos3DI128::new(1, 1, 1);
        let max = Pos3DI128::new(2, 2, 2);
        let bounds = (min, max);
        Pos3DI128::from_index(bounds, 100);
    }

    #[test]
    fn test_pos3d_i128_ix_empty() {
        let min = Pos3DI128::new(2, 2, 2);
        let max = Pos3DI128::new(1, 1, 1);
        let bounds = (min, max);
        assert_eq!(Pos3DI128::range_size(bounds), 0);
        assert!(Pos3DI128::range(bounds).next().is_none());
    }
}
