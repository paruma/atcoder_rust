...
mod test {
    ...
    fn test_doubling_random() {
        ...
        for _ in 0..500 {
            let n = rng.random_range(1..11);
            let max_k = rng.random_range(1..21);

            let f = (0..n).map(|_| rng.random_range(0..n)).collect::<Vec<_>>();

            let d = Doubling::new(&f, max_k);

            for _ in 0..100 {
                let k = rng.random_range(0..=max_k);
    ...
    fn test_doubling_with_monoid_string_random() {
        ...
        for _ in 0..100 {
            ...
            let f = (0..n).map(|_| rng.random_range(0..n)).collect::<Vec<_>>();
            let g = (0..n)
                .map(|_| ((rng.random_range(0..26) + b'a') as char).to_string())
                .collect::<Vec<_>>();
