...
- **「変換」から「作用・系内部の演算」への転換**:
    ...
    - **系内部での単位元生成 (`Sum`, `Product`)**: `0.into()` や `1.into()` の代わりに、代数系が持つ演算ルールから単位元を導き出す。具体的には `std::iter::empty::<T>().sum()` や `product()` を使用する。`num-traits` の `Zero`/`One` を避ける理由は、`ac-library-rs` の `ModInt` 等がこれらを実装していない場合があるためである。
