...
2.  **スクリプトによる検証内容**
    ...
    - **Coverage**: 行カバレッジを測定。**100%でない場合、未実行行のソースコードが前後2行のコンテキスト付きで直接表示されます**。これを見て不足しているテストケースを特定してください。`unreachable!()` のように不変条件により到達不能な行は許容します。
    - **Format**: 指定されたソースファイルだけに `rustfmt --edition 2024` を実行し、自動整形します。
